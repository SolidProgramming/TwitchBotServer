...
namespace Bot_Manager
{
    public static class BotManager
    {
        ...
        private static async Task<bool> CheckChannelLiveStatus(TwitchBotModel bot)
        {
            ...
            (ApiCallSuccess, userId) = await GetTwitchUserId(bot, bot.Settings.Channel);

            if (ApiCallSuccess)
            {
                GetStreamsResponse streamsResponse = await bot.TwitchAPI.Helix.Streams.GetStreamsAsync(userIds: new() { userId });
